...
#+begin_src yaml :tangle ./cloud-init.yml :comments none
  ...
  runcmd:
    ...
    - chmod +x /usr/local/bin/kind
      # this is only here since speed for SJC1 is slow for downloading kubectl
    - curl -L https://github.com/humacs/humacs/releases/download/0.0.1-alpha/kubectl -o /usr/local/bin/kubectl
    ...
    - |
      (
        ...
        su humacs -c 'bazel';
        su humacs -c 'git clone https://github.com/kubernetes/kubernetes /home/humacs/kubernetes';
        su humacs -c 'cd && emacs -batch -l ~/.emacs'
      ) &
    ...
    - su humacs -c 'cd /home/humacs/humacs/kind-configs && kind create cluster --config /home/humacs/humacs/kind-configs/kind-config.yaml --image kindest/node:v1.19.0'
    ...
    - su humacs -c 'kubectl -n apisnoop wait pod --for=condition=ready --selector=io.apisnoop.db=postgres --timeout=90s'
    - su humacs -c 'kubectl -n apisnoop port-forward svc/postgres 5432 2>&1 > /dev/null' &
